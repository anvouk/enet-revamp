    ...
    if (currentCommand == enet_list_end(&peer->sentReliableCommands)) {
        ...
        if (outgoingCommand == NULL) {
            outgoingCommand = enet_protocol_find_sent_reliable_command(
                &peer->outgoingSendReliableCommands, reliableSequenceNumber, channelID
            );
        }
...
enet_protocol_handle_send_reliable(
    ENetHost *host, ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData
)
{
    size_t dataLength;
...
)
{
    ...
    *currentData += dataLength;
    ...
    if (enet_peer_queue_incoming_command(
            peer, command, (const enet_uint8 *)command + sizeof(ENetProtocolSendUnreliable), dataLength, 0, 0
        )
        == NULL) {
        return -1;
    }
    ...
    *currentData += fragmentLength;
    ...
    if (startCommand == NULL) {
        ...
        startCommand = enet_peer_queue_incoming_command(
            peer, &hostCommand, NULL, totalLength, ENET_PACKET_FLAG_RELIABLE, fragmentCount
        );
        if (startCommand == NULL) {
            return -1;
        }
    ...
    if ((startCommand->fragments[fragmentNumber / 32] & (1 << (fragmentNumber % 32))) == 0) {
        ...
        memcpy(
            startCommand->packet->data + fragmentOffset,
...
enet_protocol_handle_incoming_commands(ENetHost *host, ENetEvent *event)
{
    ...
    headerSize
        = (flags & ENET_PROTOCOL_HEADER_FLAG_SENT_TIME ? sizeof(ENetProtocolHeader)
    ...
    for (;;) {
        if (currentCommand != enet_list_end(&peer->outgoingCommands)) {
            ...
            if (currentSendReliableCommand != enet_list_end(&peer->outgoingSendReliableCommands)
                && ENET_TIME_LESS(
                    ((ENetOutgoingCommand *)currentSendReliableCommand)->queueTime, outgoingCommand->queueTime
                )) {
                ...
                if (windowWrap) {
                    ...
                    if (outgoingCommand->sendAttempts < 1
                        ...
                        && (channel->reliableWindows
                                    [(reliableWindow + ENET_PEER_RELIABLE_WINDOWS - 1) % ENET_PEER_RELIABLE_WINDOWS]
        ...
        if (outgoingCommand->command.header.command & ENET_PROTOCOL_COMMAND_FLAG_ACKNOWLEDGE) {
            ...
            enet_list_insert(
                enet_list_end(&peer->sentReliableCommands), enet_list_remove(&outgoingCommand->outgoingCommandList)
            );
...
#ifdef ENET_DEBUG
                printf(
                    "peer %u: %f%%+-%f%% packet loss, %u+-%u ms round trip time, %f%% throttle, %u outgoing, %u/%u "
                    "incoming\n",
                    ...
                    currentPeer->channels != NULL ? enet_list_size(&currentPeer->channels->incomingReliableCommands)
                                                  : 0,
                    currentPeer->channels != NULL ? enet_list_size(&currentPeer->channels->incomingUnreliableCommands)
                                                  : 0
                );
#endif
